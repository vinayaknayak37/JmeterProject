...
public class SearchDRHistoryByMoboAdvancedCriteria extends AbstractESTDesktopSamplerClient {
    private static String BDT_TEST_NAME = "SearchDRHistoryByMoboAdvancedCriteria";
    private static final String CRITERION_TAG = "criterionString";
    private static Logger logger = LoggerFactory.getLogger(SearchDRHistoryByMoboAdvancedCriteria.class);
    ...
    public SampleResult runTest(JavaSamplerContext javaSamplerContext) {
        ...
        int numSearchHistoryByMoboAdvCriteria = 0;
        ...
        try{
            ...
            //tests["DeliveryRequestHistorySearchByMoboAdvancedCriteria"].record(self.deliveryRequestTest, InstrumentationFilters("getShipmentMoboHistoryByAdvancedCriteria"))
            numSearchHistoryByMoboAdvCriteria = deliveryRequestTest.getShipmentMoboHistoryByAdvancedCriteria(criteria, null);
            logger.info(" - Search history by date criteria [" + criteria.getFromReconDate().toString() + "]: [" + criteria.getToReconDate().toString() + "] hit [" + numSearchHistoryByMoboAdvCriteria + "] results");
            //grinder.statistics.forLastTest.setSuccess(1)
            logger.info("*** Test step DeliveryRequestHistorySearchByMoboAdvancedCriteria completed successfully. ***");
            //self.log("*** Test %d %s took %d milliseconds ***" % (tests["DeliveryRequestHistorySearchByMoboAdvancedCriteria"].getNumber(), tests["DeliveryRequestHistorySearchByMoboAdvancedCriteria"].getDescription(), grinder.statistics.forLastTest.time))
        } catch (Exception ex) {
            ...
            response.append("\"}, \"numSearchHistoryByMoboAdvCriteria\" : ");
            response.append(numSearchHistoryByMoboAdvCriteria);
            response.append("}");
